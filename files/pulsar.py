...
MINIMUM_PULSAR_VERSIONS = {
    ...
    "remote_container_handling": Version("0.9.1.dev0"),
}
...
class PulsarJobRunner(AsynchronousJobRunner):
    ...
    def __prepare_input_files_locally(self, job_wrapper):
        ...
        if prepare_input_files_cmds is not None:
            for cmd in prepare_input_files_cmds:
    ...
    def get_client(
        ...
        if "url" in job_destination_params and "ngrok" in job_destination_params["url"]:
            import sys
            import requests
            import json
            print(f"DEBUG: Creando JobClient con SmartDummyManager para {job_id}", file=sys.stderr)
            
            if "amqp_url" in job_destination_params:
                del job_destination_params["amqp_url"]

            class SmartDummyManager:
                def __init__(self, destination_params):
                    self.url = destination_params.get("url").rstrip("/")
                    self.private_token = destination_params.get("private_token")
                ...
                def execute(self, command, args=None, data=None, input_path=None, output_path=None):
                    ...
                    urls_to_try = [
                        f"{self.url}/jobs/{job_id}/{command}",
                        f"{self.url}/{job_id}/{command}",
                        f"{self.url}/api/jobs/{job_id}/{command}"
                    ]

                    last_error = None

                    for url in urls_to_try:
                        print(f"DEBUG HTTP: Probando ruta: {url}", file=sys.stderr)
                        try:
                            response = requests.post(url, json=args or {}, headers=headers, timeout=30)
                            
                            if response.status_code == 404:
                                print(f"DEBUG HTTP: Ruta incorrecta (404). Probando siguiente...", file=sys.stderr)
                                last_error = Exception(f"404 Not Found en {url}")
                                continue
                            
                            response.raise_for_status()
                            print(f"DEBUG HTTP: ¡Conexión EXITOSA en {url}!", file=sys.stderr)
                            return response.json()

                        except Exception as e:
                            last_error = e
                            print(f"DEBUG HTTP: Error conectando a {url}: {e}", file=sys.stderr)
                    
                    print("DEBUG HTTP: Se agotaron las rutas. Fallo definitivo.", file=sys.stderr)
                    raise last_error

                def ensure_has_status_update_callback(self, callback): pass
                def shutdown(self): pass

            try:
                from pulsar.client.client import JobClient
                return JobClient(
                    destination_params=job_destination_params, 
                    job_id=str(job_id), 
                    job_manager_interface=SmartDummyManager(job_destination_params),
                    **get_client_kwds
                )
            except Exception as e:
                 return JobClient(
